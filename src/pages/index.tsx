      ...
      <main className={`flex flex-col items-center justify-center w-full h-full p-2 md:p-4 ${!hasJoined ? 'pt-16 sm:pt-28 pb-8 sm:pb-12' : ''}`}>
        ...
        {!hasJoined ? (
          ...
          <div className="w-full h-full flex flex-col flex-grow min-h-0">
            {/* 1. Desktop Layout (Visible only on lg screens) */}
            <div className="hidden lg:flex lg:flex-row gap-4 w-full h-full lg:max-h-[800px] items-stretch animate-fade-in-up flex-grow min-h-0 relative">
              ...
              <div className="bg-white p-2 rounded-3xl shadow-xl flex-1 h-full sketch-border relative flex flex-col min-h-0 overflow-hidden shrink-0 z-0">
                ...
                <div className="p-4 flex-1 flex flex-col items-center justify-center relative overflow-hidden bg-white">
                  {meme && (
                  ...
                  {game?.status === 'ENDED' && (
                    <div className="absolute inset-0 bg-black/85 z-50 flex flex-col items-center justify-center gap-4 text-white p-4 text-center backdrop-blur-sm">
                      ...
                      <div className="bg-white text-black p-4 rounded-xl w-full max-w-md shadow-2xl">
                        ...
                        <div className="space-y-3 max-h-60 overflow-y-auto w-full px-2 font-hand">
                          {sortedPlayers.map((p, i) => (
            ...
            <div className="flex flex-col lg:hidden gap-2 sm:gap-3 w-full h-full overflow-y-auto no-scrollbar pb-6 px-0.5 sm:px-1">
              ...
              <div className="bg-white p-1.5 sm:p-2 rounded-xl sm:rounded-2xl sketch-border shadow-md flex flex-col items-center justify-center relative overflow-hidden shrink-0">
                ...
                {errorMsg && (
                  <div className="absolute top-16 left-1/2 -translate-x-1/2 z-50 animate-bounce bg-red-100 text-red-600 border-2 border-red-400 px-4 py-2 rounded-xl font-bold shadow-lg flex items-center gap-2 pointer-events-none text-xs">
                    <span>⚠️</span> {errorMsg}
                  </div>
                )}
                ...
                {game?.status === 'SELECTING' && isDrawer && (
                  <div className="absolute inset-0 bg-white/95 z-30 flex flex-col items-center justify-center gap-4 p-4 text-center">
                    <h2 className="text-lg font-bold">Choose a Word!</h2>
                    <div className="flex flex-wrap gap-2 justify-center">
                      {game.wordOptions.map(word => (
                        <button key={word} onClick={() => selectWord(word)} className="bg-blue-500 text-white px-4 py-2 rounded-lg text-base font-bold hover:bg-blue-600 transition shadow-lg">
                          {word}
                        </button>
                      ))}
                    </div>
                ...
                {game?.status === 'SELECTING' && !isDrawer && (
                  <div className="absolute inset-0 bg-white/90 z-20 flex flex-col items-center justify-center gap-4">
                    <h2 className="text-base font-bold text-center animate-pulse">{currentDrawerName} is choosing...</h2>
                  </div>
                )}

                {game?.status === 'ENDED' && (
                  <div className="absolute inset-0 bg-black/85 z-50 flex flex-col items-center justify-center gap-3 text-white p-4 text-center backdrop-blur-sm">
                    <h2 className="text-3xl font-bold text-yellow-400 animate-bounce">GAME OVER!</h2>
                    <div className="bg-white text-black p-3 rounded-xl w-full max-w-sm shadow-2xl">
                      <h3 className="text-base font-bold mb-2 border-b pb-1">Final Standings</h3>
                      <div className="space-y-2 max-h-48 overflow-y-auto w-full px-1 font-hand">
                        {sortedPlayers.map((p, i) => (
                          <div key={p.id} className={`flex flex-col p-1.5 rounded ${i === 0 ? 'bg-yellow-100 border border-yellow-400' : 'bg-gray-50 border border-gray-200'}`}>
                    ...
                    <button onClick={() => {
                      ...
                      setView('CREATE')
                      setRoomId(generateRoomId())
                    }} className="bg-blue-500 px-6 py-2 rounded-full font-bold hover:bg-blue-600 shadow-xl transition-transform hover:scale-105 border border-blue-400 text-xs mt-1">New Game 🏠</button>
                  </div>
                )}
              ...
              <div className="bg-white p-1.5 sm:p-2 rounded-xl sm:rounded-2xl sketch-border shadow-md shrink-0 flex flex-col gap-1">
                <div className="flex justify-between items-center border-b pb-1 text-xs px-1 font-hand">
                  ...
                  <div className="flex gap-2">
                    ...
                    <button onClick={() => location.reload()} className="text-red-500 text-[10px] font-bold underline flex items-center gap-0.5">
                      Leave Room 🚪
                    </button>
                  </div>
                </div>
